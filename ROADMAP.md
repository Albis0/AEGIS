...
| — | **Token-aware context budget (root fix for "message too long"):** the tools array (~8-15k tokens for 64 schemas) was never counted against the model's window — on small-window models system + tools alone overflowed → 413. New `context-budget.ts` `fitRequest()` counts system + history + tools + images against a conservative window (0.9×), then degrades gracefully: drops least-relevant tools first, trims history, always keeps the last user message; a known-oversized request is never sent. Schemas are now chosen BEFORE fitting (old order trimmed history before tools existed). Unit-tested (8 cases). |
| — | Model-picker cleanup across ALL providers (was Gemini-only): per-provider "useful model" filters in `models.ts` drop legacy generations, dated snapshots, and non-chat helpers (OpenAI gpt-3.5/davinci/dated, xAI grok-2/beta, Mistral dated/embed/ocr, Anthropic claude-1/2 + pre-3.5); each filter falls back to the unfiltered chat list if it would empty the picker. Groq stays curated-by-Groq, Ollama shows the user's own installed models unfiltered. Unit-tested (5 cases). |
| — | Vision hardening: the `screenshot` tool de-dupes its own double-call (same question within 4s reuses the last description — no second "captured your screen" notice, no wasted vision call) and clips the description to 2k chars before it enters history. Scoped to the tool only, so computer-use keeps getting fresh captures for its verify loop. |
| — | **Tool ↔ history consistency (fixes "attempted to call tool X which was not in request.tools"):** providers reject a request whose message history references a tool absent from `request.tools` — happens when the model calls a tool that later gets trimmed / reactively dropped / was hallucinated (e.g. `steam_screenshot` while a game is running). Before every send, referenced tools are added back from the full registry; unfindable ones have their tool_calls (and orphaned tool results) stripped from history. When tools are dropped to zero on a too-long retry, all tool_calls are stripped too. `referencedToolNames`/`dropDanglingToolCalls`/`stripAllToolCalls` + `findToolSchemaByName`, unit-tested. |
| — | **Reactive overflow recovery (the real "message too long" fix):** the proactive budget only works if `caps.contextWindow` is accurate; an unknown/renamed Groq model defaults to 131k while its real limit may be 8k → the request still 413'd. Now Groq (and the trial proxy) treat the provider's "too long" as ground truth and retry with escalating shrink — drop all tools → keep system+last-user only → hard-truncate contents — so the error can never surface regardless of registry accuracy. Plus always-on `[budget]` instrumentation (model/window/estIn/tools) for diagnosis. `keepEssential`/`truncateContents` in `context-budget.ts`, unit-tested. |
| — | **Hybrid tool routing (BM25 recovery):** shared `bm25.ts` (real BM25 w/ IDF + tokenizer, reused by knowledge RAG) + `tool-router.ts`. `getAllToolSchemas` now, when the hand-kept root lists match NO domain group, asks BM25 (built from each group's roots + tool names + descriptions) to recover the missed domain — fixing the "flaky tool selection" bug where a tool was silently not offered. Gated on zero root matches so it's purely additive: tool-selection eval stays 100% (40/40). An LLM-router fallback (`llmRouteGroups`) for BM25-ambiguous cases is implemented + unit-tested but not yet wired into the live chat path (avoids adding LLM calls to the hot path unattended). |
