...
export const codeToolSchemas: ChatCompletionTool[] = [
    ...
    // Faz CC-7 — review the working-tree diff and return structured findings.
    {type:"function",function:{name:"review_changes",description:"Review the current git changes and return a list of concrete problems (file, line, severity). Use when the user says 'review my changes', 'look over my diff', 'any bugs in what I changed?'. Reviews unstaged changes by default; set staged=true for staged, or base to diff against a branch/commit.",parameters:{type:"object",properties:{repo_path:{type:"string",description:"Repo path (default: current directory)"},staged:{type:"boolean",description:"true = review staged changes (git diff --staged)"},base:{type:"string",description:"Diff against this branch/commit instead (e.g. 'main')"}},additionalProperties:false}}},
