// AEGIS — Model capability registry ("per-model tailoring")
// =============================================================================
// Goal: each AI model does only as much as it SUPPORTS, and we NEVER send a
// parameter it doesn't support. This eliminates errors like 400/422
// "unsupported parameter", "max_tokens too large", and "model does not support
// tools/vision" at the root.
//
// Data compiled from each provider's OFFICIAL docs (June 2026):
...
//
// Unknown model → SAFE default (based on provider type). The goal is to avoid
// errors; when unsure we stay on the restrictive side (e.g. keep max_tokens
// small, disable vision).

export interface ModelCaps {
    /** does it support function calling / tool use */
    supportsTools: boolean;
    /** is the temperature parameter accepted (reasoning models often don't) */
    supportsTemperature: boolean;
    /** highest accepted temperature (Anthropic=1, most=2) — for clamping */
    maxTemperature: number;
    /** does it support a separate "system" role/instruction (o1-mini/o1-preview don't) */
    supportsSystemPrompt: boolean;
    /** does it support image input */
    supportsVision: boolean;
    /** does it support SSE streaming */
    supportsStreaming: boolean;
    /** is it a reasoning (thinking) model */
    reasoning: boolean;
    /** OpenAI o-series/gpt-5: Chat Completions wants max_completion_tokens instead of max_tokens */
    usesMaxCompletionTokens: boolean;
    /** completion (output) token cap — a max_tokens EXCEEDING this returns 400 */
    maxOutputTokens: number;
    /** total context window (tokens) — for the history-trimming budget */
    contextWindow: number;
}

const K = 1024;

// Safe, conservative baseline. Every unknown model starts from this.
function base(overrides: Partial<ModelCaps> = {}): ModelCaps {
    return {
        ...
        maxOutputTokens: 4096,   // safest baseline (many older models are capped at 4096)
        contextWindow: 32 * K,
        ...overrides,
    };
}
...
function openaiCaps(id: string): ModelCaps {
    ...
    // o1 / o3 / o4 series: HAS tools, NO temperature, max_completion_tokens.
    ...
    // gpt-5* (except the chat variant): reasoning, no temperature, max_completion_tokens.
    ...
    // Unknown OpenAI → has tools, no vision, safe 4K output.
    return base({supportsTools: true, contextWindow: 128 * K, maxOutputTokens: 4 * K});
}

// ───────────────────────────────────────────────────────────── Anthropic
// All support tools + vision + a separate system, with a temperature cap of 1.
// The only real pitfall: the max_output cap (the 8192 default returns 400 on a low-cap model).
function anthropicCaps(id: string): ModelCaps {
    ...
    if (/claude-3-opus|claude-3-haiku|claude-3-sonnet/.test(id)) return a(4 * K);         // 3.0 family → 4096
...
function geminiCaps(id: string): ModelCaps {
    ...
    // 3.x series (3 / 3.1 / 3.5 Pro+Flash) reasoning; 1M ctx, 64K output.
    if (/gemini-3/.test(id)) {
        return base({supportsTools: true, supportsVision: true, reasoning: true, contextWindow: 1000 * K, maxOutputTokens: 64 * K});
    }
    // 2.5 series reasoning/thinking; tools + vision + system + large output.
    ...
    // Unknown Gemini → tools + vision, 1M ctx, 8K output.
    return base({supportsTools: true, supportsVision: true, contextWindow: 1000 * K, maxOutputTokens: 8 * K});
}
...
function mistralCaps(id: string): ModelCaps {
    ...
    if (/codestral-mamba|open-mistral-7b|mistral-7b|mistral-tiny/.test(id)) {
        // Old/small → tool support is unreliable.
...
function deepseekCaps(id: string): ModelCaps {
    // V4 (deepseek-v4-flash / deepseek-v4-pro) — has tools + temperature, 128K ctx.
    // deepseek-chat/reasoner is being retired on 2026-07-24; v4-flash enables thinking
    // mode via an API parameter rather than the model name, so reasoning=false is safe here.
    ...
    if (/reasoner|r1/.test(id)) {
        // R1 / deepseek-reasoner: NO tools, NO temperature/top_p/penalty. max 64K (incl. CoT).
        return base({
            reasoning: true, supportsTools: false, supportsTemperature: false,
            contextWindow: 64 * K, maxOutputTokens: 8 * K,
        });
    }
    // deepseek-chat (V3): has tools, has temperature.
...
function ollamaCaps(id: string, numCtx: number): ModelCaps {
    // Tool support is only enabled for known tool-compatible families (otherwise Ollama returns 400).
    ...
    return base({
        ...
        supportsStreaming: false,                  // the app calls Ollama with stream:false
...
/**
 * Provider + model ID → capabilities. ollamaNumCtx is only meaningful for Ollama.
 */
...
// ───────────────────────────────────────────────────────────── Helpers (used by callAI)

/** Rough token estimate (~4 chars/token; an image ~1100 tokens). */
export function estimateTokens(text: string): number {
    return Math.ceil(text.length / 4);
}

/** Clamp the output tokens to send based on the model. */
...
/** Adjust/drop the temperature based on the model. */
